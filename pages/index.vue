<template>
<div class="container relative mx-auto mt-16 max-w-screen-lg text-gray-900 md:px-4">
  <CustomAlert v-if="content" @click.native.self="content = null" @scroll.native="content = null" :content="content" />
  <div class="md:grid md:grid-cols-2">
    <div>
      <header class="px-4">
        <div class="logo w-32" v-html="require(`~/assets/icons/logo.svg?include`)"></div>
      ...
      <div id="steps" class="px-4">
        <div id="step-1" class="mt-16">
          ...
          <div class="field-container">
            <ImageLoader :content="images" type="logo" :resizeImage="resizeImage" label="Attach brand logo" description="recommended format: svg or png" :showAlert="showAlert" />
            <ImageLoader :content="images" type="photo" :resizeImage="resizeImage" label="Attach photo" description="recommended: a square photo" :showAlert="showAlert" />
          </div>
        </div>
        <div id="step-2" class="mt-16">
          ...
          <div class="field-container mt-6">
            ...
            <input id="fullname" spellcheck="false" type="text" v-model="businessInfo.name" class="mt-2 px-4 w-full h-12 bg-gray-200 placeholder-gray-600 rounded focus:outline-none" />
          </div>
          <div class="field-container mt-6">
            <label for="job-title" class="ml-4">Job title</label>
            <input id="job-title" type="text" spellcheck="true" v-model="businessInfo.jobTitle" class="mt-2 px-4 w-full h-12 bg-gray-200 placeholder-gray-600 rounded focus:outline-none" />
          </div>
          <div class="field-container mt-6">
            <label for="business-name" class="ml-4">Business name</label>
            <input id="business-name" spellcheck="false" type="text" v-model="businessInfo.businessName" class="mt-2 px-4 w-full h-12 bg-gray-200 placeholder-gray-600 rounded focus:outline-none" />
          </div>
          <div class="field-container relative mt-6">
            <label for="business-description" class="flex justify-between ml-4">Business description
              <span v-if="businessInfo.businessDescription" class="mr-4" :class="
                    descriptionCharCount <= 160
                      ? 'text-green-600'
                      : 'text-orange-600'
                  ">{{ descriptionCharCount }}</span></label>
            <textarea id="business-description" :value="businessInfo.businessDescription" @input="businessInfo.businessDescription = $event.target.value"
              class="block mt-2 px-4 py-3 w-full bg-gray-200 placeholder-gray-600 rounded focus:outline-none resize-none" rows="5" placeholder="Please keep it short and simple ( <160 characters )"></textarea>
          </div>
          <div class="field-container relative mt-6">
            <label for="pgp-public-key" class="flex justify-between ml-4">Public key<span v-if="businessInfo.publicKey" class="mr-4" :class="pubKeyIsValid ? 'text-green-600' : 'text-orange-600'">{{ pubKeyIsValid ? '' : 'Invalid schema' }}</span>
            </label>
            <textarea id="pgp-public-key" v-model="businessInfo.publicKey" class="block mt-2 px-4 py-3 w-full bg-gray-200 placeholder-gray-600 rounded focus:outline-none resize-none" rows="5" spellcheck="false"
              placeholder="Paste the PGP PUBLIC KEY BLOCK here"></textarea>
          </div>
          <div class="field-container relative mt-6">
            <label for="pgp-public-key" class="flex justify-between ml-4">Public key fingerprint<span v-if="businessInfo.fingerprint" class="mr-4"
                :class="fingerprintIsValid ? 'text-green-600' : 'text-orange-600'">{{ fingerprintIsValid ? '' : 'Invalid fingerprint' }}</span>
            </label>
            <input id="pgp-fingerprint" spellcheck="false" type="text" v-model="businessInfo.fingerprint" class="mt-2 px-4 w-full h-12 bg-gray-200 placeholder-gray-600 rounded focus:outline-none"
        ...
        <div id="step-3" class="mt-16">
          ...
          <ButtonInput v-for="(item, index) in primaryActions" :key="index" name="primaryActions" :type="primaryActions" :item="item" :index="index" :buttonBg="colors.buttonBg.color" :removeAction="removeAction" />
          <div class="field-container flex flex-wrap">
            <button v-for="(action, index) in actions.primaryActions" @click="addAction('primaryActions', index)" class="p-3 flex-shrink-0 bg-gray-900 mt-6 mr-6 rounded-full cursor-pointer shadow" :style="{
                  backgroundColor: `${colors.buttonBg.color}`,
                }" :title="action.name" :aria-label="'Add ' + action.name">
        ...
        <div id="step-4" class="mt-16">
          ...
          <ButtonInput v-for="(item, index) in socialLinks" :key="index" name="socialLinks" :type="socialLinks" :item="item" :index="index" :removeAction="removeAction" />
          <div class="field-container flex flex-wrap">
            <button v-for="(action, index) in actions.socialLinks" @click="addAction('socialLinks', index)" class="p-3 flex-shrink-0 mt-6 mr-6 rounded-full cursor-pointer shadow" :style="{ backgroundColor: action.color }" :title="
        ...
        <div id="step-5" class="mt-16">
          ...
          <div class="field-container">
            <FeaturedContentLoader :featured="featured" type="images" :multiple="true" :resizeImage="resizeImage" label="Attach image" mimetypes="image/jpeg, image/png" description="supports jpeg, png" :showAlert="showAlert" />
            <FeaturedContentLoader :featured="featured" type="music" :resizeImage="resizeImage" label="Attach music" mimetypes="audio/mpeg" description="supports mp3" :showAlert="showAlert" />
            <FeaturedContentLoader :featured="featured" type="videos" :resizeImage="resizeImage" label="Attach video" mimetypes="video/mp4,video/webm" description="supports mp4, webm" :showAlert="showAlert" />
            <FeaturedContentLoader :featured="featured" type="documents" :resizeImage="resizeImage" label="Attach document" mimetypes="application/pdf" description="supports pdf" :showAlert="showAlert" />
          </div>
        </div>
        <div id="step-6" class="mt-16">
          <h2 class="font-extrabold text-2xl">Colour customisation</h2>
          <div class="field-container">
            <ColourSelector name="logoBg" label="Logo background" :colors="colors" />
            <ColourSelector name="buttonBg" label="Button background" :colors="colors" />
          </div>
        </div>
        ...
        <div id="step-7" class="mt-16">
          ...
          <div class="field-container mt-6">
            <div class="relative inline-block w-24 mr-3 align-middle select-none transition duration-200 ease-in">
              <input type="checkbox" name="toggle" aria-label="Toggle footer credit" id="toggle" v-model="footerCredit"
              ...
              <label for="toggle" class="toggle-label block overflow-hidden h-12 rounded-full bg-gray-900 cursor-pointer transition-colors duration-200 shadow"></label>
            </div>
            <label for="toggle">{{
    ...
    <div id="preview-container" class="relative w-full sm:h-full mt-20">
      <div id="preview" class="flex flex-col items-center justify-center sm:sticky sm:top-0">
        <div id="device" class="bg-gray-900 device-curve rounded-lg w-full sm:max-w-sm shadow-lg sm:mt-10 sm:mb-20">
          <h2 class="text-2xl text-center py-3 font-extrabold text-gray-200">
            Preview
          </h2>
          <div id="browserFrame" class="overflow-hidden rounded-lg mx-2 pb-8 flex flex-col">
            <div id="topBar" class="topbar bg-white shadow-sm z-10">
              <div id="searchField" class="pt-6 pl-2 pb-2 flex items-center">
                <input type="text" class="pl-4 h-12 w-full bg-gray-200 rounded" aria-label="vCard URL" disabled :value="'yoursite/vcard/' + username + '/'" />
                <div class="w-6 mx-4" v-html="require(`~/assets/icons/ellipsis.svg?include`)"></div>
              </div>
            </div>
            <DBizCard ref="html" :username="username" :businessInfo="businessInfo" :images="images" :featured="featured" :colors="colors" :primaryActions="primaryActions" :socialLinks="socialLinks" :PreviewMode="PreviewMode"
              :downloadVcard="downloadVcard" :footerCredit="footerCredit" :showAlert="showAlert" :shareHasLightBG="shareHasLightBG" :actionHasLightBG="actionHasLightBG" :publicKey="businessInfo.publicKey" :downloadKey="downloadKey"
  ...
  <div id="downloadSection" class="mt-16 px-4">
    ...
    <div class="field-container">
      <CheckListItem v-for="(item, index) in downloadCheckList" :downloadCheckList="downloadCheckList" :item="item" :key="index" :index="index" />
      <button ref="downloadPackage" @click="downloadPackage" class="inline-block bg-gray-900 text-lg uppercase font-extrabold text-white px-8 py-6 rounded-full mt-12 shadow-md select-none transition-colors duration-200" :class="
            downloadChecked
  ...
  <footer class="py-8 mx-4 mt-24">
    <p class="text-gray-700 text-sm">
      <a class="underline hover:text-green-600 transition-colors duration-200" href="http://t.me/dbizcard" target="_blank" rel="noopener noreferrer">Join the Telegram group</a>
      for reporting issues, suggestions and feedback.
    </p>
    <hr class="border-t-2 border-gray-200 mt-4" />
    <div class="flex justify-start items-center mt-8">
      <div class="logo w-16 mr-2 self-start" v-html="require(`~/assets/icons/logo.svg?include`)"></div>
      <div>
        ...
        <p class="text-gray-700 text-xs">
          by
          <a class="underline hover:text-green-600 transition-colors duration-200" href="https://www.vishnuraghav.com/" target="_blank" rel="noopener noreferrer">Vishnu&nbsp;Raghav</a>
          | AGPLv3&nbsp;License |
          <a class="underline hover:text-green-600 transition-colors duration-200" href="https://github.com/vishnuraghavb/dbizcard" target="_blank" rel="noopener noreferrer">View&nbsp;Source</a>
        </p>
      </div>
    </div>
  </footer>
...
export default {
  ...
  data() {
    return {
      downloadCheckList: [ {
        ...
        {
          label: 'I have verified that all the links are working correctly in the Preview',
          checked: false,
        },
        {
          label: 'I have verified that all the information provided here are correct',
          checked: false,
        },
      ],
      images: {
  ...
  computed: {
    ...
    fingerprintIsValid() {
      return this.businessInfo.fingerprint && this.businessInfo.fingerprint.match( /^[a-zA-Z0-9]{40}$/ )
    },
    downloadChecked() {
